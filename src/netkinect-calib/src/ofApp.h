...
public:
    ofApp()
            : minR_("Min Radius", .1, .01, 1), maxR_("Max Radius", .3, .1, 1),
              trackingEnabled_("Enable Tracking", false), globalCalibration_("Global Calibration", true),
              error_("Modell Error", .01, .001, .2), percent_("Inlier %", .99, .2, 1),
              resolution_("Resolution", .1, .005, 1000), samples_("# Samples", 10000, 10, 50000),
              passMin_("Z Min", .01, .01, 8), passMax_("Z Max", 2.5, .01, 800), meanSamples_("Smoothing Frame", 3, 1, 60),
              background_("Background", 127, 0, 255), movementThreshold_("Movement threshold", 15, .1, 50),
              icpDistanceThreshold_("Distance Threshold ", 2, .001, 3), icpIterations_("# Iterations", 500, 20, 1000),
              iaMinSampleDistance__("Min Sample Distance", .05, .001, 1),
              iaMaxCorrespondenceDistance_("Max correspondence distance", 3, .1, 8),
              iaIterations_("Max Iterations", 10000, 10, 50000), xTrans_("X Translation", 0, -5, 5),
              yTrans_("Y Translation", 0, -5, 5), zTrans_("Z Translation", 0, -5, 5), xRot_("X Rotation", 0, -180, 180),
              yRot_("Y Rotation", 0, -180, 180), zRot_("Z Rotation", 0, -180, 180) {}
    ...
    void computeFeatures(pcl::PointCloud<pcl::PointXYZ>::Ptr refCloud,
                         pcl::PointCloud<pcl::FPFHSignature33>::Ptr refFeatures);
